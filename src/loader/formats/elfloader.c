...
{
    // We only support little endian (not bit endian)
...
{
    // Here we are dealing with executable files not shared libraries
    // Those executable files should be present at correct place as mentioned in our virtual address.
    // eg. some program has virtual add. lower than the mentioned one, it may be possible that it may loaded at stack
...
{
    // The very start of the elf file is elf header.
    return file->elf_memory;
}
...
{
    // This if condition, because program header is optional.
    if(header->e_phoff == 0)
    {
        return 0;
    }
...
{
    // Here we find lowest base address among all headers.
...
// Here we process all the headers and process them.
